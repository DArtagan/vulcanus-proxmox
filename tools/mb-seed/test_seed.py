...
def book(**overrides: object) -> dict[str, Any]:
    base = {
...
def dict_of(pairs: list[tuple[str, str]]) -> dict[str, Any]:
...
class TestArtistCredit(unittest.TestCase):
    """The credit is seeded as separate indexed names, in this library's phrasing.

    That phrasing is the convention already established across its releases in
    MusicBrainz: "author narrated by A, B & C".
    ...
    def test_a_single_narrator_is_a_separate_indexed_name(self) -> None:
        seeded = fields()
    ...
    def test_three_narrators_use_commas_then_an_ampersand(self) -> None:
    ...
    def test_a_full_cast_credits_every_narrator(self) -> None:
        credit = seed.artist_credit(
    ...
    def test_no_narrator_credits_the_author_alone(self) -> None:
    ...
    def test_no_author_seeds_no_credit_at_all(self) -> None:
        self.assertEqual(seed.artist_credit(book(author=None)), [])

    def test_credit_indices_are_consecutive_from_zero(self) -> None:
...
class TestNarratorParsing(unittest.TestCase):
    def test_splits_on_commas_and_strips(self) -> None:
        self.assertEqual(
            seed.narrators(book(narrator=" Jeff Hays ,Annie Ellicott ")),
            ["Jeff Hays", "Annie Ellicott"],
        )

    def test_absent_narrator_is_an_empty_list(self) -> None:
        self.assertEqual(seed.narrators(book(narrator=None)), [])

    def test_trailing_separator_does_not_produce_a_blank_name(self) -> None:
        self.assertEqual(seed.narrators(book(narrator="Jeff Hays,")), ["Jeff Hays"])

    def test_drops_placeholders_that_are_not_people(self) -> None:
        # The narrator tag of "Stain: Everybody Loves Large Chests, Vol. 8" ends
        # with Gary Furlong, Soundbooth Theater and "full cast". Seeding "full
        # cast" would create a junk artist in a public database.
        self.assertEqual(
            seed.narrators(book(narrator="Jeff Hays, full cast")), ["Jeff Hays"]
        )

    def test_drops_placeholders_case_insensitively(self) -> None:
        self.assertEqual(
            seed.narrators(book(narrator="Jeff Hays, Full Cast")), ["Jeff Hays"]
        )

    def test_keeps_a_studio_which_is_a_real_entity(self) -> None:
        self.assertIn(
            "Soundbooth Theater",
            seed.narrators(book(narrator="Jeff Hays, Soundbooth Theater")),
        )

    def test_a_dropped_placeholder_is_reported_for_review(self) -> None:
        self.assertEqual(
            seed.dropped_narrators(book(narrator="Jeff Hays, full cast")), ["full cast"]
        )

    def test_nothing_is_reported_when_every_narrator_is_a_person(self) -> None:
        self.assertEqual(seed.dropped_narrators(book()), [])


class TestReleaseShape(unittest.TestCase):
    def test_is_an_audiobook_release(self) -> None:
        self.assertEqual(fields()["type"], ["Other", "Audiobook"])

    def test_is_one_track_holding_the_whole_book(self) -> None:
        seeded = fields()
    ...
    def test_track_title_matches_the_release_title(self) -> None:
        seeded = fields()
        self.assertEqual(seeded["mediums.0.track.0.name"], seeded["name"])

    def test_an_audible_download_has_no_barcode(self) -> None:
        self.assertEqual(fields()["barcode"], "none")

    def test_seeds_the_release_date_in_parts(self) -> None:
        seeded = fields()
    ...
    def test_a_year_only_date_seeds_only_the_year(self) -> None:
        seeded = fields(date={"year": 2022})
        self.assertEqual(seeded["events.0.date.year"], "2022")
        self.assertNotIn("events.0.date.month", seeded)

    def test_an_undated_book_seeds_no_event(self) -> None:
        self.assertNotIn("events.0.date.year", fields(date=None))

    def test_an_unmapped_language_is_omitted_rather_than_guessed(self) -> None:
        self.assertNotIn("language", fields(language=None))

    def test_seeds_the_audible_product_url(self) -> None:
    ...
    def test_leaves_the_link_type_for_the_editor_to_choose(self) -> None:
    ...
    def test_seeds_no_url_without_an_asin(self) -> None:
        self.assertNotIn("urls.0.url", fields(asin=None))

    def test_seeds_no_url_for_an_asin_of_unknown_provenance(self) -> None:
    ...
    def test_an_unknown_provenance_asin_is_still_recorded_in_the_annotation(
        self,
    ) -> None:
    ...
    def test_never_seeds_an_amazon_url(self) -> None:
        ...
        for _, value in seed.seed_fields(book()):
            self.assertNotIn("amazon.", value)

    def test_every_seeded_value_is_a_string(self) -> None:
        for key, value in seed.seed_fields(book()):
            self.assertIsInstance(value, str, key)

    def test_no_empty_values_are_seeded(self) -> None:
        for key, value in seed.seed_fields(book(publisher=None, series=None)):
            self.assertNotEqual(value, "", key)


class TestProvenance(unittest.TestCase):
    def test_the_edit_note_cites_the_asin(self) -> None:
        self.assertIn("B08M3S7ZPF", fields()["edit_note"])

    def test_the_edit_note_does_not_claim_audible_for_an_unattributed_asin(
        self,
    ) -> None:
    ...
    def test_the_edit_note_survives_a_missing_asin(self) -> None:
        note = fields(asin=None)["edit_note"]
        self.assertIn("audiobook file", note)
        self.assertNotIn("None", note)

    def test_the_annotation_records_the_asin_and_series(self) -> None:
    ...
    def test_narrators_are_never_repeated_in_the_annotation(self) -> None:
    ...
    def test_a_bare_book_seeds_no_annotation(self) -> None:
        self.assertNotIn(
...
class TestCoverArtStep(unittest.TestCase):
    """The cover art step automates everything the Cover Art Archive allows.

    It has no upload API and no form seeding, so that is: know whether art is
    needed, have the image ready as a real file, and land on the right page.
    """

    def test_links_to_the_add_cover_art_page_for_the_release(self) -> None:
        self.assertEqual(
    ...
    def test_the_upload_link_opens_in_a_new_tab(self) -> None:
    ...
    def test_the_upload_link_cannot_reach_back_through_the_opener(self) -> None:
        markup = seed.cover_art_link("93d306d6-a23a-4bec-bcb3-3098f8f25ac7", "Upload")
        self.assertIn("noopener", markup)


class TestSeedPage(unittest.TestCase):
    """The hand-off to MusicBrainz has to be a real link.

    Only a link can be middle-clicked into a new tab. A submit button cannot
    be, and seeding is documented as POST only — so the link points at a local
    page that posts.
    """

    def test_the_handoff_is_an_anchor_not_a_submit_button(self) -> None:
        markup = seed.seed_link(3, "Open in MusicBrainz")
        self.assertIn('href="/seed/3"', markup)
        self.assertTrue(markup.lstrip().startswith("<a "))

    def test_the_link_is_not_forced_into_a_new_tab(self) -> None:
        # target="_blank" would take the choice away again.
        self.assertNotIn("target=", seed.seed_link(3, "Open in MusicBrainz"))

    def test_the_posting_page_targets_musicbrainz(self) -> None:
        markup = seed.seed_form(book())
        self.assertIn('action="https://musicbrainz.org/release/add"', markup)
        self.assertIn('method="POST"', markup)

    def test_the_posting_page_carries_every_seeded_field(self) -> None:
        markup = seed.seed_form(book())
        for key, _ in seed.seed_fields(book()):
            self.assertIn(f'name="{key}"', markup)
        self.assertIn("Eden Hudson", markup)

    def test_the_posting_page_carries_the_redirect(self) -> None:
    ...
    def test_the_posting_page_submits_itself(self) -> None:
        self.assertIn("submit()", seed.seed_form(book()))

    def test_the_posting_page_works_without_javascript(self) -> None:
...
class TestSaveCover(unittest.TestCase):
    """The page tells the user where the file is, so it is there before it says so.

    Writing it as a side effect of the browser fetching the thumbnail made the
    claim true only if that request happened to succeed.
    """
    ...
    def save(
        self, fetcher: Callable[..., bytes | None], **overrides: object
    ) -> Path | None:
    ...
    def test_writes_the_file_and_returns_its_path(self) -> None:
        path = self.save(lambda *_: b"\xff\xd8\xffdata")
        self.assertEqual(path, self.directory / "B08M3S7ZPF.jpg")
        self.assertEqual(path.read_bytes(), b"\xff\xd8\xffdata")

    def test_creates_the_directory(self) -> None:
        nested = self.directory / "covers"
        seed.save_cover(book(), nested, "/audio/import", "pod", fetcher=lambda *_: b"x")
        self.assertTrue(nested.is_dir())

    def test_returns_none_when_the_art_cannot_be_read(self) -> None:
        self.assertIsNone(self.save(lambda *_: None))

    def test_writes_nothing_when_the_art_cannot_be_read(self) -> None:
        self.save(lambda *_: None)
        self.assertEqual(list(self.directory.iterdir()), [])

    def test_does_not_refetch_a_file_it_already_has(self) -> None:
        calls = []

        def counting(*_: object) -> bytes:
            calls.append(1)
            return b"data"
    ...
    def test_a_book_with_no_art_is_not_fetched(self) -> None:
        called = []
        path = self.save(
            lambda *_: called.append(1) or b"x",
...
class TestLedger(unittest.TestCase):
    ...
    def test_records_and_persists(self) -> None:
